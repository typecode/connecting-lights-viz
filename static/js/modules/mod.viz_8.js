var Viz_Eight = function(options) {
	...
	fn = {
		...
		message_added: function(message){
			...
			message.marker = L.circle(message.lat_lng, 200, {
					stroke: false,
		...
		update_particles: function(){
			...
			for(i = 0; i < internal.messages.length; i++){
				...
				if(my_bounds.contains(my_particle.lat_lng)){
					my_particle.marker.setLatLng(my_particle.lat_lng);
				}
			}
		},
